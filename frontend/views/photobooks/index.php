...
$this->title = Yii::t('app','Мои проекты');
        ...
        <div  class="row">
            <div class="col-xs-3">
                ...
                <?php foreach($sidemenus as $key=>$menuitem): ?>
                    ...
                    <a href="<?php echo Yii::$app->urlManager->createUrl(['photobooks/index', 'status'=>$menuitem['status']]); ?>" class="list-group-item <?php if($status==$menuitem['status']) echo 'active'; ?>">
    ...
    const  STATUS_ARCHIVE = 10;
                                            ...
                                            <div class="col-xs-4">
                                                ...
                                                <div class="photobook-index-tools">
                                                    ...
                                                    <a class="pull-right tooltips"  href="<?php echo Url::toRoute(['photobooks/view-book', 'ref'=>AlphaId::id($order->user_id), 'id'=>AlphaId::id($order->id)]); ?>" >
                                                        <?php echo Yii::t('app', 'Посмотреть'); ?>
                                                    </a><br/>

                                                    <?php if($order->status==Photobook::STATUS_NEW ||
                                                             $order->status==Photobook::STATUS_READY_SENT_TO_CLIENT ||
                                                             $order->status==Photobook::STATUS_WAIT_EDIT_FROM_CUSTOMER ||
                                                             $order->status==Photobook::STATUS_SENT_TO_PRINT):
                                                    ?>

                                                    <a class="pull-right tooltips"  href="<?php echo Url::toRoute(['photobooks/edit', 'ref'=>AlphaId::id($order->user_id), 'id'=>AlphaId::id($order->id)]); ?>" >
                                                    ...
                                                    <?php if(
                                                             $order->status!=Photobook::STATUS_READY_SENT_TO_PRINT &&
                                                             $order->status!=Photobook::STATUS_READY &&
                                                             $order->status!=Photobook::STATUS_READY_SENT_TO_CLIENT &&
                                                             $order->status!=Photobook::STATUS_READY_FOR_PRINT_WAIT_PAYMENT &&
                                                    ...
                                                    <a class="pull-right tooltips btnSendToCustomer"  href="#" data-url="<?php echo Url::toRoute(['photobook-api/prepare-get-link-dialog', 'id'=>AlphaId::id($order->id), 'ref'=>AlphaId::id($order->user_id)]); ?>" data-ref="<?php echo AlphaId::id($order->user_id); ?>" data-id="<?php echo AlphaId::id($order->id); ?>" >
                                                        <?php echo Yii::t('app', 'Отправить на согласование'); ?>
                                                    </a><br/>
                                                    <?php endif; ?>


                                                    <?php if($order->status==Photobook::STATUS_WAIT_EDIT_FROM_CUSTOMER): ?>
                                                    <a class="pull-right tooltips" target="_blank"  href="<?php echo Url::toRoute(['photobooks/view', 'key'=> $order->view_access_key], true); ?>"   >
                                                        <?php echo Yii::t('app', 'Посмотреть комментарии клиента'); ?>
                                                    </a><br/>
                                                    <?php endif; ?>
                                                    ...
                                                    $order->status==Photobook::STATUS_SENT_TO_PRINT ||
                                                    $order->status==Photobook::STATUS_WAIT_EDIT_FROM_CUSTOMER ||
                                                        $order->status==Photobook::STATUS_SENT_TO_CUSTOMER):
                                                    ?>
                                                    <a class="pull-right tooltips"  href="<?php echo Url::toRoute(['photobooks/send-to-print', 'ref'=>AlphaId::id($order->user_id), 'id'=>AlphaId::id($order->id)]); ?>"   >
                                                        <?php echo Yii::t('app', 'Отправить в печать'); ?>
                                                    </a><br/>
                                                    ...
                                                    <a class="pull-right tooltips"  href="#"  data-ref="<?php echo AlphaId::id($order->user_id); ?>" data-id="<?php echo AlphaId::id($order->id); ?>" >
                                                        <?php echo Yii::t('app', 'Оставить отзыв'); ?>
                                                    </a><br/>
                                                    ...
                                                    ?>
                                                        <a class="pull-right tooltips"  href="<?php echo Url::toRoute(['photobooks/get-invoice',  'pb_id'=>$order->id]); ?>" >
                                                            <?php echo Yii::t('app', 'Оплатить счет'); ?>
                                                    ...
                                                    <?php if($order->status==Photobook::STATUS_RECEIVED_FEEDBACK): ?>

                                                    <a class="pull-right tooltips"  href="#" data-ref="<?php echo AlphaId::id($order->user_id); ?>" data-id="<?php echo AlphaId::id($order->id); ?>"  >
                                                        <?php echo Yii::t('app', 'Отправить в архив'); ?>
                                                    </a><br/>
